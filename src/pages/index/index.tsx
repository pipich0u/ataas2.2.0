...
const SidebarIcon = ({ name }: { name: 'dashboard' | 'cluster' | 'modelRepo' | 'deploy' | 'image' | 'resource' | 'benchmark' | 'engine' | 'logs' | 'playground' | 'apiKey' | 'user' | 'engineMgr' }) => {
  const white = '#fff';
  return (
...
const AtAasDesign = () => {
  ...
  const [expandedGroups, setExpandedGroups] = useState<Set<string>>(() => new Set(['概览', '通用管理', '模型管理', '模型测试', '身份权限', '系统监控']));
  ...
  const handleDeployLog = (item: DeployServiceItem, logId: number) => {
    message.info('查看日志: ' + item.name + ' - ' + (item.modelInfo.logs.find((l: { id: number; name: string }) => l.id === logId)?.name || ''));
  };
  ...
  const getNodeDeployServices = (node: NodeRecord) => {
    return deployServices.filter((service) => {
      ...
      return works.some((work: string) => work === nodeName || work.includes(nodeName) || nodeName.includes(work));
    });
  };
  ...
  const getScheduleDetailNodes = (item: DeployServiceItem | null) => {
    if (!item) return [];
    const names = item.modelInfo.works.split(',').map((name: string) => name.trim().toLowerCase()).filter(Boolean);
    return deployNodes.filter((node) => names.some((name: string) => name === node.name.toLowerCase()));
  };
  ...
  const SIDEBAR_ITEMS = [
    ...
    { key: 'nodes', icon: <NodeIndexOutlined className="ataas-sidebar-icon" />, label: '节点管理' },
    { key: 'modelRepo', icon: <SidebarIcon name="modelRepo" />, label: '模型仓库' },
    { key: 'deploy', icon: <SidebarIcon name="deploy" />, label: '模型部署' },
    { key: 'startupTemplates', icon: <SidebarIcon name="engine" />, label: '启动模板' },
    ...
    { key: 'engines', icon: <SidebarIcon name="engineMgr" />, label: '引擎管理' },
  ];
  ...
  const SIDEBAR_GROUPS = [
    ...
    { title: '模型测试', items: getSidebarItems(['playgroundChat', 'benchmark']) },
                          ...
                          <div key={item.id} className="ataas-model-repo-card" onClick={() => setModelRepoDetail(item)}>
                            ...
                            <div className="ataas-model-repo-card-foot">
                              ...
                              <div>
                                ...
                                >
                                  <Button size="small" className="ataas-model-repo-more-button" onClick={(e) => e.stopPropagation()}>更多</Button>
                                </Dropdown>
                              </div>
                            </div>
                          </div>
            ...
            <div className="ataas-section-stack">
              <div className="ataas-panel ataas-startup-template-page ataas-deploy-list">
                <div className="ataas-deploy-list-toolbar">
                  <Input.Search
                    className="ataas-deploy-list-search ataas-startup-template-search"
                    placeholder="搜索模板名称"
                ...
                <div className="ataas-startup-template-grid">
                  {filteredStartupTemplates.map((template) => (
                    <div key={template.key} className="ataas-startup-template-card">
                      <div className="ataas-startup-template-card-glow" />
                      <div className="ataas-startup-template-card-head">
                        <div>
                          <strong>{template.name}</strong>
                        </div>
                      </div>
                      <div className="ataas-startup-template-card-tags">
                        <span>官方</span>
                        <span>{template.hardware}</span>
                        <span>{template.engine}</span>
                        <span>{template.cardCount} 卡</span>
                      </div>
                      <p>{template.description || '暂无描述'}</p>
                      <div className="ataas-startup-template-card-foot">
            ...
            <div className="ataas-section-stack">
              ...
              <div className="ataas-panel ataas-log-page ataas-deploy-list">
                <div className="ataas-log-toolbar ataas-deploy-list-toolbar">
                  ...
                  <Input.Search className="ataas-deploy-list-search ataas-log-search" placeholder={'搜索' + ({ all: '日志', user: '操作人', action: '操作', object: '对象', status: '状态' }[logSearchField] || '日志') + '...'} value={logSearchText} onChange={(e) => setLogSearchText(e.target.value)} allowClear size="middle" />
                </div>
                <div className="ataas-deploy-table-wrap ataas-log-table-wrap">
                  <Table dataSource={filteredLogs} rowKey={(_, i) => String(i)} columns={logColumns} scroll={{ x: 1000 }} pagination={{ pageSize: 10, showTotal: (t) => `共 ${t} 条`, showSizeChanger: true }} />
                </div>
              </div>
              </ConfigProvider>
            </div>
              ...
              { title: '操作', key: 'action', width: 120, fixed: 'right', render: (_, r) => (
                r.status !== '已恢复' ? (
                  <div className="ataas-alert-table-actions">
                    <Button className="ataas-alert-action-button" type="text" size="small" onClick={() => { setAlertList((prev: AlertRecord[]) => prev.map((a: AlertRecord) => a.key === r.key ? { ...a, status: '已恢复' } : a)); message.success('已恢复: ' + r.target); }}>恢复</Button>
                  </div>
                ) : (
              ...
              <div className="ataas-section-stack">
                ...
                <div className="ataas-panel ataas-alert-page ataas-deploy-list">
                  ...
                  <div className="ataas-deploy-table-wrap ataas-alert-table-wrap">
                    <Table dataSource={filteredAlerts} rowKey="key" columns={alertColumns} scroll={{ x: 1280 }} pagination={{ pageSize: 10, showTotal: (t) => `共 ${t} 条`, showSizeChanger: true }} />
                  </div>
                </div>
                </ConfigProvider>
              </div>
...
<Drawer title={`节点详情 - ${clusterNodeModalTitle}`} open={clusterNodeModal} onClose={() => { setClusterNodeModal(false); setClusterNodeRecord(null); }} width={640}>
      ...
      <Drawer className="ataas-deploy-drawer" title="模型部署" open={deployDrawerOpen} onClose={() => setDeployDrawerOpen(false)} width={560}>
                      ...
                      <div className="ataas-pd-deploy-stack">
                        <div className="ataas-pd-template-strip">
                          <div className="ataas-pd-form-row">
                            ...
                            <div className="ataas-pd-form-control ataas-pd-template-strip-main">
                              <Select value={pdSelectedTemplateKey || undefined} onChange={(value) => applyPdTemplate(value as string)} placeholder="选择 PD 模板（可选）" options={pdTemplateOptions} allowClear />
                              <Tooltip title="上传 PD 模板 YAML">
                                <Button icon={<UploadOutlined />} onClick={() => setPdTemplateUploadOpen(true)} />
                              </Tooltip>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="ataas-pd-section">
                          ...
                          <div className="ataas-pd-section-body">
                            ...
                            {renderPdShellPanel('router', 'Routher YAML', pdRouterShellText, setPdRouterShellText, setPdRouterParams)}
                          </div>
                        </div>
                        {/* PD Worker 配置 */}
      ...
      <Modal
        ...
        footer={<Button type="primary" onClick={() => setPdTemplateUploadOpen(false)}>完成</Button>}
        width={680}
      >
        <div className="ataas-pd-template-upload-modal">
          <Upload.Dragger
            accept=".yaml,.yml"
            multiple={false}
            showUploadList={false}
            beforeUpload={(file) => {
              const reader = new FileReader();
              reader.onload = (ev) => {
                setPdRouterUploadedYaml(ev.target?.result as string || '');
          ...
          >
            ...
            <p className="ant-upload-hint">{pdRouterUploadedYaml ? '已上传，点击可重新选择' : '点击或拖拽文件到这里'}</p>
          </Upload.Dragger>
          <Upload.Dragger
            accept=".yaml,.yml"
            multiple={false}
            ...
            beforeUpload={(file) => {
              const reader = new FileReader();
              reader.onload = (ev) => {
                setPdPrefillUploadedYaml(ev.target?.result as string || '');
                setPdTemplateMode('upload');
                setPdSelectedTemplateKey('');
                setPdPrefillTemplateKey('');
                setPdDecodeTemplateKey('');
          ...
          >
            ...
            <p className="ant-upload-hint">{pdPrefillUploadedYaml ? '已上传，点击可重新选择' : '点击或拖拽文件到这里'}</p>
          </Upload.Dragger>
        </div>
      </Modal>
      ...
      {deployDetailItem && (() => {
        ...
        const mainColumns = [
          ...
          { title: '状态', dataIndex: 'main', key: 'main', render: (_: any) => <span className="ataas-deploy-detail-status-running"><CheckCircleFilled />运行中</span> },
        ];
        ...
        return (
          ...
          >
            <div className="ataas-deploy-detail-legacy" style={{ padding: '4px 0' }}>
              <div className="ataas-deploy-detail-section-title"><span>基础信息</span></div>
              <div className="ataas-deploy-detail-basic-grid" style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 24 }}>
              ...
              <div className="ataas-deploy-detail-section-title"><span>模型信息</span></div>
              <div style={{ marginBottom: 16 }}>
              ...
              <div className="ataas-deploy-detail-model-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px 24px' }}>
              ...
              {isPdMode ? (
                  ...
                  expandable={{
                    expandedRowRender: (record: any) => {
                      ...
                      const subColumns = [
                        ...
                        { title: '状态', dataIndex: 'status', key: 'status', width: 100, render: (_: string) => <span className="ataas-deploy-detail-status-running"><CheckCircleFilled />运行中</span> },
                      ...
                      const podSuffix = (record.node || '-').toLowerCase().replace(/[^a-z0-9]/g, '-');
                      const subData = [
                        { key: 'router', podName: `router-${podSuffix}-0`, comp: <span style={{ color: '#4e5969', fontWeight: 500 }}>Router</span>, machine: routerNodeNames, gpu: '-', status: 'running', logId: 1 },
                        { key: 'prefill', podName: `prefill-${podSuffix}-0`, comp: <span style={{ color: '#4e5969', fontWeight: 500 }}>Prefill</span>, machine: prefillNodeNames, gpu: '8 卡', status: 'running', logId: 2 },
                        { key: 'decode', podName: `decode-${podSuffix}-0`, comp: <span style={{ color: '#4e5969', fontWeight: 500 }}>Decode</span>, machine: decodeNodeNames, gpu: '8 卡', status: 'running', logId: 3 },
                      ];
                ...
                <Table
                  dataSource={dataSource.map((record: any) => {
                    ...
                    return {
                      key: record.key,
                      podName: `pod-${podSuffix}-0`,
                      comp: <span style={{ color: '#4e5969', fontWeight: 500 }}>推理</span>,
                      machine: record.node,
                      gpu: deployDetailItem.modelInfo.vram || '-',
                      status: 'running',
                      logId: record.key + 1,
                    };
                  })}
      ...
      {deployDetailItem?.deployMode === 'PD 分离' && (
        <Modal className="ataas-add-instance-modal" title="添加实例" open={addInstanceModalOpen} onCancel={() => setAddInstanceModalOpen(false)} width={720} footer={
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
            ...
            <Button type="primary" onClick={handleAddInstanceConfirm}>确认添加</Button>
          </div>
        }>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {/* Router */}
            <div className="ataas-pd-section">
              ...
              <div className="ataas-pd-section-body">
                <div className="ataas-pd-section-row">
                  <div>
                    <div className="ataas-pd-field-label">节点选择</div>
                    <div className="ataas-pd-node-selector" onClick={() => { setAddInstNodePickerMode('router'); setAddInstNodePickerSelected([...addInstRouterNodes]); setAddInstNodePickerOpen(true); }}>
                ...
                <div className="ataas-pd-mode-bar">
                  <Segmented value={addInstRouterParamMode} onChange={(v) => setAddInstRouterParamMode(v as 'template' | 'manual')} size="small" options={[{ value: 'template', label: 'YAML 模版' }, { value: 'manual', label: '手填参数' }]} />
                  <div style={{ flex: 1 }}>
                    {addInstRouterParamMode === 'template' ? (
                      ...
                      <div className="ataas-pd-shell-area">
            ...
            <div className="ataas-pd-section">
              ...
              <div className="ataas-pd-section-body">
                <div className="ataas-pd-section-row">
                  <div>
                    <div className="ataas-pd-field-label">节点选择</div>
                    <div className="ataas-pd-node-selector" onClick={() => { setAddInstNodePickerMode('prefill'); setAddInstNodePickerSelected([...addInstPrefillNodes]); setAddInstNodePickerOpen(true); }}>
                ...
                <div className="ataas-pd-mode-bar">
                  <Segmented value={addInstPrefillParamMode} onChange={(v) => setAddInstPrefillParamMode(v as 'template' | 'manual')} size="small" options={[{ value: 'template', label: 'YAML 模版' }, { value: 'manual', label: '手填参数' }]} />
                  <div style={{ flex: 1 }}>
            ...
            <div className="ataas-pd-section">
              ...
              <div className="ataas-pd-section-body">
                <div className="ataas-pd-section-row">
                  <div>
                    <div className="ataas-pd-field-label">节点选择</div>
                    <div className="ataas-pd-node-selector" onClick={() => { setAddInstNodePickerMode('decode'); setAddInstNodePickerSelected([...addInstDecodeNodes]); setAddInstNodePickerOpen(true); }}>
                ...
                <div className="ataas-pd-mode-bar">
                  <Segmented value={addInstDecodeParamMode} onChange={(v) => setAddInstDecodeParamMode(v as 'template' | 'manual')} size="small" options={[{ value: 'template', label: 'YAML 模版' }, { value: 'manual', label: '手填参数' }]} />
                  <div style={{ flex: 1 }}>
      ...
      {deployDetailItem && deployDetailItem.deployMode !== 'PD 分离' && (
        <Modal title="添加实例" open={addInstanceModalOpen} onCancel={() => setAddInstanceModalOpen(false)} width={600} footer={
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
            <Button onClick={() => setAddInstanceModalOpen(false)}>取消</Button>
            <Button type="primary" onClick={handleAddInstanceConfirm}>确认添加</Button>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div className="ataas-pd-section">
              <div className="ataas-pd-section-header">
                <SettingOutlined className="ataas-pd-section-header-icon" style={{ color: '#722ed1' }} />
                <span>实例配置</span>
              </div>
              <div className="ataas-pd-section-body">
                <div className="ataas-pd-section-row">
                  <div>
                    ...
                    <div className="ataas-pd-node-selector" onClick={() => { setAddInstNodePickerMode('router'); setAddInstNodePickerSelected([...addInstRouterNodes]); setAddInstNodePickerOpen(true); }}>
                      {addInstRouterNodes.length > 0 ? addInstRouterNodes.map((k) => {
                        const n = deployNodes.find((d) => d.key === k);
                        return n ? <Tag key={k} closable onClose={(e) => { e.stopPropagation(); setAddInstRouterNodes((prev) => prev.filter((x) => x !== k)); }} className="ataas-pd-node-tag">{n.name}</Tag> : null;
                      }) : <span className="ataas-pd-node-placeholder">点击选择节点</span>}
                    </div>
                  </div>
                </div>
                <div className="ataas-pd-mode-bar">
                  ...
                  <div style={{ flex: 1 }}>
                    {addInstRouterParamMode === 'template' ? (
                      ...
                      <div className="ataas-pd-params-area">
                        {addInstRouterParams.length === 0 ? (
                          <div className="ataas-pd-param-empty">暂无参数</div>
                        ) : (
                          addInstRouterParams.map((param, pi) => (
                            <div key={pi} className="ataas-pd-param-row">
                              <Input placeholder="参数名" value={param.key} onChange={(e) => { const next = [...addInstRouterParams]; next[pi] = { ...next[pi], key: e.target.value }; setAddInstRouterParams(next); }} style={{ flex: 1 }} size="small" />
                              <Input placeholder="参数值" value={param.value} onChange={(e) => { const next = [...addInstRouterParams]; next[pi] = { ...next[pi], value: e.target.value }; setAddInstRouterParams(next); }} style={{ flex: 1 }} size="small" />
                              <Button icon={<DeleteOutlined />} danger size="small" onClick={() => setAddInstRouterParams(addInstRouterParams.filter((_, j) => j !== pi))} />
                            </div>
                          ))
                        )}
                        <div className="ataas-pd-param-add">
                          <Button size="small" icon={<PlusOutlined />} onClick={() => setAddInstRouterParams([...addInstRouterParams, { key: '', value: '' }])}>添加参数</Button>
                        </div>
                      </div>
                    )}
                  </div>
      ...
      {/* 添加实例 - 节点选择弹窗 */}
      <Modal title={'选择' + (addInstNodePickerMode === 'router' ? 'Router' : addInstNodePickerMode === 'prefill' ? 'Prefill' : 'Decode') + '节点'} open={addInstNodePickerOpen} onCancel={() => setAddInstNodePickerOpen(false)} footer={
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          <Button onClick={() => setAddInstNodePickerOpen(false)}>取消</Button>
          <Button type="primary" onClick={() => {
            if (addInstNodePickerMode === 'router') setAddInstRouterNodes([...addInstNodePickerSelected]);
            else if (addInstNodePickerMode === 'prefill') setAddInstPrefillNodes([...addInstNodePickerSelected]);
            else if (addInstNodePickerMode === 'decode') setAddInstDecodeNodes([...addInstNodePickerSelected]);
        ...
        <Table
          ...
          rowSelection={{
            ...
            onSelect: (record) => { setAddInstNodePickerSelected((prev) => prev.includes(record.key) ? prev.filter((k) => k !== record.key) : [...prev, record.key]); },
            onSelectAll: (selected, _selectedRows, changeRows) => { setAddInstNodePickerSelected((prev) => { const changeKeys = changeRows.map((r) => r.key); if (selected) return [...new Set([...prev, ...changeKeys])]; return prev.filter((k) => !changeKeys.includes(k)); }); },
          }}
          columns={[
            { title: '节点名称', dataIndex: 'name', key: 'name', render: (v, r) => <><strong>{v}</strong><span className="ataas-table-sub">{r.ip}</span></> },
            { title: '显卡类型', dataIndex: 'gpuType', key: 'gpuType', render: (v) => <Tag>{v}</Tag> },
