              ...
              return (
                <div key={item.id} className="ataas-deploy-service-card">
                  ...
                  <div className="ataas-deploy-service-meta-grid">
                    ...
                    <div>运行时间 <span>{getRuntimeText(item)}</span></div>
                    <div>显存占用 <span>{item.modelInfo.vram}</span></div>
                    <div>精度 <span>{item.modelInfo.point}</span></div>
                    <div>自动重启 <span><em>{item.modelInfo.restartCount}</em> / {item.modelInfo.restartNumber || item.modelInfo.restartCount}</span></div>
                    <div>并发控制 <span>{item.modelInfo.concurrencyControllCount}</span></div>
                  </div>
