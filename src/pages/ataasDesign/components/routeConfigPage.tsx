...
import { routeConfigStore, useRouteConfigStore, type RouteAuthMode as AuthMode, type RouteMatchType as MatchType, type RouteType, type SharedRouteRecord as RouteRecord } from './routeConfigStore';
...
const RouteConfigPage = () => {
  ...
  const serviceMeshExits = useMemo<ServiceMeshExitRecord[]>(() => {
    ...
    return Array.from(grouped.entries()).map(([name, linkedRoutes]) => ({
      id: `st1-se-${name}`,
      name,
      cluster: 'ST1',
      host: `${name}.internal.dns`,
      targetService: linkedRoutes[0]?.service || `${name}-svc.production.svc:8000`,
      ingressNames: linkedRoutes.map((route) => route.name),
      status: linkedRoutes.some((route) => route.enabled) ? 'Running' : 'Disabled',
    }));
  }, [routes]);
  ...
  const filteredServiceMeshExits = useMemo(() => {
    ...
    return serviceMeshExits.filter((item) => [item.name, item.cluster, item.host, item.targetService, ...item.ingressNames].some((value) => value.toLowerCase().includes(query)));
  ...
  const columns: ColumnsType<RouteRecord> = [
    ...
    { title: '请求授权', dataIndex: 'auth', key: 'auth', width: 140, render: (auth: AuthMode) => authLabels[auth] },
    {
      ...
      render: (enabled: boolean, route) => <span className="route-config-status"><Switch size="small" checked={enabled} onChange={(checked) => routeConfigStore.update(route.id, { enabled: checked })} /><i className={enabled ? 'enabled' : ''}>{enabled ? '已启用' : '已停用'}</i></span>,
    },
    {
  ...
  return (
    <div className="route-config-page">
      ...
      <header className="route-config-toolbar">
        ...
        <div className="route-config-toolbar-actions">
          {resourceView === 'ingress' ? (
            <Button type="primary" icon={<PlusOutlined />} onClick={openCreate}>创建服务网格入口</Button>
          ) : (
            <Button icon={<PlusOutlined />} onClick={() => window.dispatchEvent(new CustomEvent('ataas:navigate', { detail: { tab: 'operationsCanvas' } }))}>前往画布创建出口</Button>
          )}
        </div>
      </header>
      ...
      <section className="route-config-card ataas-panel ataas-log-page ataas-deploy-list">
        <div className="route-config-table ataas-deploy-table-wrap ataas-log-table-wrap">
          {resourceView === 'ingress' ? <Table<RouteRecord>
            ...
            expandable={{
              ...
              expandedRowRender: (route) => <div className="route-config-policy-detail">
                <div className="route-config-policy-detail-head"><strong>策略配置</strong><strong>策略描述</strong></div>
          ...
          /> : <div className="route-config-exit-grid">
            {filteredServiceMeshExits.length ? filteredServiceMeshExits.map((exit) => (
              <article className="route-config-exit-card" key={exit.id}>
                <header>
                  <div><strong>{exit.name}</strong><small>ServiceEntry · {exit.cluster}</small></div>
                  <span className={`route-config-exit-status ${exit.status.toLowerCase()}`}><i />{exit.status === 'Running' ? '运行中' : '已停用'}</span>
                </header>
                <div className="route-config-exit-field"><span>Hosts</span><code title={exit.host}>{exit.host}</code></div>
                <div className="route-config-exit-field"><span>目标服务</span><code title={exit.targetService}>{exit.targetService}</code></div>
                <footer>
                  <span>关联服务网格入口 · {exit.ingressNames.length}</span>
                  <div className="route-config-ingress-tags">{exit.ingressNames.map((name) => <Tag key={name}>{name}</Tag>)}</div>
                </footer>
              </article>
      ...
      >
        <div className="route-config-policy-list">
          {routePluginCatalog.map((plugin) => {
            const enabled = plugin.key in policyDraft;
            const fields = ROUTE_PLUGIN_CONFIG_SCHEMAS[plugin.key] || [];
            return <section className={`route-config-policy-plugin ${enabled ? 'enabled' : ''}`} key={plugin.key}>
              <header><span><strong>{plugin.name}</strong><small>{plugin.description}</small></span><Switch checked={enabled} onChange={(checked) => setPolicyDraft((current) => checked ? { ...current, [plugin.key]: buildRoutePluginConfigDefaults(plugin.key) } : Object.fromEntries(Object.entries(current).filter(([key]) => key !== plugin.key)))} /></header>
              {enabled && fields.length > 0 && <div className="route-config-policy-fields">{fields.map((field) => {
                const value = policyDraft[plugin.key]?.[field.key];
                const updateValue = (next: RoutePluginConfigValue) => setPolicyDraft((current) => ({ ...current, [plugin.key]: { ...current[plugin.key], [field.key]: next } }));
