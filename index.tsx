...
const SidebarIcon = ({ name }: { name: 'dashboard' | 'cluster' | 'modelRepo' | 'deploy' | 'image' | 'imageModel' | 'visionModel' | 'embedding' | 'rerank' | 'monitor' | 'benchmark' | 'engine' | 'template' | 'alert' | 'logs' | 'playground' | 'apiKey' | 'user' | 'engineMgr' }) => {
  const white = '#fff';
  return (
...
const StartupTemplateManager = ({ templates, setTemplates }: StartupTemplateManagerProps) => {
  ...
  const [benchmarkRows, setBenchmarkRows] = useState<Array<{ label: string; len: number; prefill: number; decode: number }>>([{ label: '1K', len: 1024, prefill: 0, decode: 0 }]);
  const [singleBenchmarkRows, setSingleBenchmarkRows] = useState<Array<{ inputLen: number; outputLen: number; concurrency: number; ttft: number; tpot: number; tps: number }>>([{ inputLen: 1024, outputLen: 512, concurrency: 1, ttft: 0, tpot: 0, tps: 0 }]);
  ...
  const openEditor = (template?: StartupTemplateRecord) => {
    ...
    setBenchmarkRows((next?.benchmark || [{ label: '', len: 0, prefill: 0, decode: 0 }, { label: '', len: 0, prefill: 0, decode: 0 }]).map((row) => ({
      label: row.label || String(row.len || ''),
      len: row.len || 0,
      prefill: row.prefill || 0,
      decode: row.decode || 0,
    })));
    setSingleBenchmarkRows((next?.benchmark || [{ inputLen: 1024, outputLen: 512, concurrency: 1, ttft: 0, tpot: 0, tps: 0 }, { inputLen: 0, outputLen: 0, concurrency: 0, ttft: 0, tpot: 0, tps: 0 }]).map((row) => ({
      inputLen: row.inputLen ?? row.len ?? 1024,
      outputLen: row.outputLen ?? 512,
  ...
  const handleEditorTypeChange = (type: 'single' | 'pd' | 'kt') => {
    ...
    setImportedBenchmarkSource(undefined);
    importedBenchmarkSourceRef.current = undefined;
    setBenchmarkRows([{ label: '', len: 0, prefill: 0, decode: 0 }, { label: '', len: 0, prefill: 0, decode: 0 }]);
    setSingleBenchmarkRows([{ inputLen: 1024, outputLen: 512, concurrency: 1, ttft: 0, tpot: 0, tps: 0 }, { inputLen: 0, outputLen: 0, concurrency: 0, ttft: 0, tpot: 0, tps: 0 }]);
  ...
  const submitEditor = () => {
    ...
    const benchmark = type === 'single'
      ? singleBenchmarkRows.map((row) => ({ ...row, label: `${row.inputLen}/${row.outputLen}` }))
    ...
    const next: StartupTemplateRecord = {
      ...
      model: values.model || values.name,
      modelFamily: (values.model || values.name).split('-')[0] || values.model || values.name,
      description: values.description || values.name,
      scenario: values.description || values.name,
      gpu: values.gpu || '-',
      gpuCount: type === 'pd' ? 1 : Number(values.gpuCount || 1),
      hardware: inferChipVendor(values.gpu) === 'NVIDIA' ? `NVIDIA ${values.gpu}` : values.gpu,
      engine: values.engine,
      quantization: values.quantization,
      deployMode,
      nodeCount: type === 'pd' ? 2 : 1,
      cardCount: type === 'pd' ? 1 : Number(values.gpuCount || 1),
  ...
  const renderKtLineChart = (template: StartupTemplateRecord) => {
    const rows = (template.benchmark || []).filter((point) => point.prefill !== undefined && point.decode !== undefined);
    if (!rows.length) return null;
    const width = 1180;
    const height = 300;
    const pad = { top: 20, right: 58, bottom: 34, left: 62 };
    const plotWidth = width - pad.left - pad.right;
    const plotHeight = height - pad.top - pad.bottom;
  ...
  const renderBenchmark = (template: StartupTemplateRecord) => (
    <div className="ataas-template-benchmark">
      ...
      {template.type === 'single' && template.source === 'official' ? (() => {
        ...
        return (
          <>
            <div className="ataas-template-compare-chart-wrap">{renderSingleCompareChart(rows)}</div>
            <Table
              size="small"
              pagination={false}
  ...
  const renderKtOfficialDetails = (template: StartupTemplateRecord) => {
    ...
    const chartHeight = Math.max(180, Math.round(Math.max(tableHeight - 28, 150) * (300 / 266)));
    return (
        ...
        <section className="ataas-template-kt-box ataas-template-kt-metrics-panel">
          ...
          <div className="ataas-template-kt-metrics-grid">
            ...
            <div className="ataas-template-kt-chart-panel">
              ...
              <div className="ataas-template-kt-chart-wrap" style={{ height: chartHeight }}>{renderKtLineChart(template)}</div>
            </div>
          </div>
        </section>
        {notes.length > 0 && (
    ...
    return (
      <div className="ataas-template-expand">
        <div className="ataas-template-detail-grid">
          <div className="ataas-template-detail-panel">
            <div className="ataas-template-detail-title">引擎参数</div>
            <pre>{template.command}</pre>
          </div>
          <div className="ataas-template-detail-panel">
            <div className="ataas-template-detail-title">测试环境</div>
  ...
  return (
    ...
    <div className="ataas-template-manager">
                ...
                return (
                  <div key={template.key} className={`ataas-template-card ${template.type === 'pd' ? 'pd-card' : ''} ${expanded ? 'expanded' : ''}`}>
                    <button type="button" className="ataas-template-card-main" onClick={() => canExpand && setExpandedKey(expanded ? '' : template.key)}>
                      <div>
                        ...
                        <div className="ataas-template-tags">
                          <Tag>{template.source === 'official' ? '官方' : '自建'}</Tag>
                    ...
                    <div className="ataas-template-card-actions">
                      ...
                      <Button size="small" type="primary" onClick={() => setDeployTarget(template)}>部署</Button>
                    </div>
          ...
          {scenePool.length === 0 ? <div className="ataas-template-scene-tag-empty">暂无场景标签</div> : scenePool.map((tag) => {
            ...
            return (
              <div className="ataas-template-scene-tag-row" key={tag}>
                ...
                <Button danger size="small" onClick={() => deleteSceneTag(tag)}>删除</Button>
              </div>
            );
          })}
        </div>
      ...
      <Modal className="ataas-template-model-service-import-modal" title="从模型服务导入" open={modelServiceImportOpen} onCancel={() => setModelServiceImportOpen(false)} footer={null} width={560}>
        <div className="ataas-template-benchmark-import-note">选择当前平台运行中的模型服务，同步模型、引擎、量化、实例数和资源信息；启动参数和 GPU 类型仍需在模板中确认。</div>
      ...
      >
        <Form form={form} layout="vertical" className="ataas-template-editor-form">
          <div className="ataas-template-form-section"><strong>1. 基本信息</strong><div className="ataas-template-form-grid">
            <Form.Item label="模板类型" name="type" rules={[{ required: true }]}><Select disabled={Boolean(editing)} onChange={handleEditorTypeChange} options={[{ value: 'single', label: '单机模板' }, { value: 'pd', label: 'PD模板' }, { value: 'kt', label: 'KT模板 (KTransformers)' }]} /></Form.Item>
            <Form.Item label="模板名称" name="name" rules={[{ required: true }]}><Input placeholder="如 DeepSeek-R1 H20 单机模板" /></Form.Item>
            <Form.Item label={editorType === 'pd' ? '模型' : '模型 *'} name="model" rules={editorType === 'pd' ? [] : [{ required: true, message: '请填写模型名称' }]}><AutoComplete options={[...new Set(richTemplates.map((item) => item.model || item.modelFamily).filter(Boolean))].map((value) => ({ value, label: value }))} placeholder="输入模型名称实时筛选" filterOption={(inputValue, option) => String(option?.value || '').toLowerCase().includes(inputValue.toLowerCase())} /></Form.Item>
          ...
          <div className="ataas-template-form-section"><strong>2. 硬件与引擎</strong>
            {(editorType === 'single' || editorType === 'kt') && <div className="ataas-template-import-row"><Button size="small" onClick={openModelServiceImport}>从模型服务导入</Button></div>}
            <div className="ataas-template-form-grid three">
              <Form.Item label={editorType === 'pd' ? 'GPU 类型' : 'GPU 类型 *'} name="gpu" rules={editorType === 'pd' ? [] : [{ required: true, message: '请选择 GPU 类型' }]}><Select showSearch options={['H20-96G', 'H100-80G', 'A100-80G', 'A800-80G', '910B-64G'].map((value) => ({ value, label: value }))} placeholder="请选择" /></Form.Item>
              <Form.Item label="驱动版本" name="driverVersion"><Input placeholder="550.54.15 / CUDA 12.4" /></Form.Item>
              {editorType !== 'pd' && <Form.Item label="GPU 数量 *" name="gpuCount"><InputNumber min={1} max={64} style={{ width: '100%' }} /></Form.Item>}
            </div>
            {editorType === 'kt' && <div className="ataas-template-form-grid three">
              <Form.Item label="CPU 型号 *" name="cpuModel"><Input placeholder="2x AMD EPYC 9355" /></Form.Item>
              <Form.Item label="内存 *" name="ktMem"><Input placeholder="768Gi" /></Form.Item>
              <Form.Item label="KT 版本" name="ktVersion"><Input placeholder="0.5.1" /></Form.Item>
            </div>}
            <div className="ataas-template-form-grid">
              <Form.Item label={editorType === 'pd' ? '推理引擎' : '推理引擎 *'} name="engine"><Select options={['SGLang', 'vLLM', 'TensorRT-LLM', 'KTransformers'].map((value) => ({ value, label: value }))} placeholder="请选择" /></Form.Item>
            </div>
            {editorType !== 'pd' && <Form.Item className="ataas-template-shell-field" label="启动参数 *" name="command" rules={[{ required: true, message: '请粘贴启动参数' }]}><Input.TextArea className="ataas-template-shell-input" rows={6} placeholder="--model /models/xxx --tp 8 --mem-fraction-static 0.85 --max-total-tokens 32768 ..." onChange={(event) => handleCommandChange(event.target.value)} /></Form.Item>}
          {parseHint && <div className="ataas-template-parse-hint">{parseHint}</div>}</div>
          {editorType === 'pd' && <div className="ataas-template-form-section"><strong>3. PD YAML *</strong>
          ...
          {editorType !== 'pd' && <div className="ataas-template-form-section"><strong>3. 测试数据</strong>
            {editorType === 'single' && <div className="ataas-template-import-row"><Button size="small" onClick={openBenchmarkImport}>从性能压测导入</Button></div>}
            ...
            <Table className="ataas-template-benchmark-table" size="small" pagination={false} scroll={{ x: 700 }} rowKey={(_, index) => String(index)} dataSource={singleBenchmarkRows} columns={[
              ...
              { title: '', width: 48, fixed: 'right', render: (_, __, index) => <Button type="text" danger icon={<DeleteOutlined />} onClick={() => setSingleBenchmarkRows((prev) => prev.length <= 1 ? prev : prev.filter((_, i) => i !== index))} /> },
            ]} />
            <Button type="link" onClick={() => setSingleBenchmarkRows((prev) => [...prev, { inputLen: 1024, outputLen: 512, concurrency: 1, ttft: 0, tpot: 0, tps: 0 }])}>添加一行数据</Button>
            </> : <>
            <Table className="ataas-template-benchmark-table" size="small" pagination={false} scroll={{ x: 520 }} rowKey={(_, index) => String(index)} dataSource={benchmarkRows} columns={[
              { title: '标签', width: 120, render: (_, row, index) => <Input value={row.label} onChange={(event) => setBenchmarkRows((prev) => prev.map((item, i) => i === index ? { ...item, label: event.target.value } : item))} /> },
              { title: '长度', width: 100, render: (_, row, index) => <InputNumber value={row.len} onChange={(value) => setBenchmarkRows((prev) => prev.map((item, i) => i === index ? { ...item, len: Number(value || 0) } : item))} /> },
              ...
              { title: '', width: 48, fixed: 'right', render: (_, __, index) => <Button type="text" danger icon={<DeleteOutlined />} onClick={() => setBenchmarkRows((prev) => prev.length <= 1 ? prev : prev.filter((_, i) => i !== index))} /> },
            ]} />
            <Button type="link" onClick={() => setBenchmarkRows((prev) => [...prev, { label: '', len: 0, prefill: 0, decode: 0 }])}>添加一行数据</Button>
            </>}
          </div>}
        </Form>
      </Drawer>
...
const AtAasDesign = () => {
  ...
  const deployEngineOptions = useMemo(() => {
    return engineList
  ...
  useEffect(() => {
    if (deployEngine && !deployEngineOptions.some((option) => option.value === deployEngine)) {
      setDeployEngine(undefined);
    }
  }, [deployEngine, deployEngineOptions]);
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'startupTemplates': return (
            <div className="ataas-section-stack">
              <StartupTemplateManager templates={startupTemplates} setTemplates={setStartupTemplates} />
            </div>
          );
      ...
      {deployDetailItem && deployDetailModalOpen && (() => {
        ...
        const pdFlatRows = isPdMode
          ? dataSource.flatMap((record: any) => {
            ...
            return [
              { key: `${record.key}-router`, instance: record.instance, instanceRecord: record, podName: `router-${suffix}-0`, comp: 'Router', machine: routerNodeNames, gpu: '-', logId: 1 },
            ...
            <div className="ataas-deploy-detail-legacy" style={{ padding: '4px 0' }}>
              ...
              {isPdMode ? (
                  ...
                  columns={[
                    {
                      ...
                      key: 'instance',
                      width: 118,
                      render: (_value: string, row: any, index: number) => ({
                        children: detailInstanceCell(row.instanceRecord || row),
                ...
                <Table
                  ...
                  columns={[
                    { title: '实例', dataIndex: 'instance', key: 'instance', width: 118, render: (_value: string, row: any) => detailInstanceCell(row.instanceRecord || row) },
                    { title: 'Pod 名称', dataIndex: 'podName', key: 'podName' },
                    { title: '所选机器', dataIndex: 'machine', key: 'machine' },
                    { title: '显卡数量', dataIndex: 'gpu', key: 'gpu', width: 90 },
                    { title: '状态', dataIndex: 'status', key: 'status', width: 100, render: detailStatus },
          ...
          <div className="ataas-pd-deploy-stack ataas-add-instance-pd-stack">
            ...
            <div className="ataas-pd-section">
              ...
              <div className="ataas-pd-section-body">
                ...
                {renderPdShellPanel('router', 'Routher YAML', addInstRouterShellText, setAddInstRouterShellText, setAddInstRouterParams)}
              </div>
            </div>
            {/* PD Worker */}
            <div className="ataas-pd-section ataas-pd-worker-card">
